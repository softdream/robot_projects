...
#ifndef CSPCLIDAR_DRIVER_H
#define CSPCLIDAR_DRIVER_H
#include <stdlib.h>
#include <atomic>
#include <map>
#include "serial.h"
...
#ifndef __cplusplus
#error "The CSPC LIDAR SDK requires a C++ compiler to be built"
#endif
#endif
...
class CSPClidarDriver {
  ...
  */
  virtual ~CSPClidarDriver();

  /*!
  * @brief 连接雷达 \n
  * 连接成功后，必须使用::disconnect函数关闭
  * @param[in] port_path    串口号
  * @param[in] baudrate    波特率，CSPC LIDAR-SS雷达波特率：
  *     230400 G2-SS-1
  * @return 返回连接状态
  * @retval 0     成功
  * @retval < 0   失败
  * @note连接成功后，必须使用::disconnect函数关闭
  * @see 函数::CSPClidarDriver::disconnect (“::”是指定有连接功能,可以看文档里的disconnect变成绿,点击它可以跳转到disconnect.)
  */
