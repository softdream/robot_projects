...
CSPClidarDriver::CSPClidarDriver():
  _serial(NULL) {
  isConnected         = false;
  isScanning          = false;
  //串口配置参数
...
result_t CSPClidarDriver::connect(const char *port_path, uint32_t baudrate) {
  ScopedLocker lk(_serial_lock);
  m_baudrate = baudrate;
...
void CSPClidarDriver::flushSerial() {
  if (!isConnected) {
    return;
  }
...
void CSPClidarDriver::disconnect() {
  isAutoReconnect = false;
...
void CSPClidarDriver::disableDataGrabbing() {
  {
    if (isScanning) {
      isScanning = false;
      _dataEvent.set();
...
bool CSPClidarDriver::isscanning() const {
  return isScanning;
}
bool CSPClidarDriver::isconnected() const {
  return isConnected;
}

result_t CSPClidarDriver::sendCommand(uint8_t cmd, const void *payload,
                                    size_t payloadsize) {
...
result_t CSPClidarDriver::sendData(const uint8_t *data, size_t size) {
  if (!isConnected) {
    return RESULT_FAIL;
  }
...
result_t CSPClidarDriver::getData(uint8_t *data, size_t size) {
  if (!isConnected) {
    return RESULT_FAIL;
  }
...
result_t CSPClidarDriver::waitResponseHeader(lidar_ans_header *header,
    uint32_t timeout) {
  int  recvPos     = 0;
...
result_t CSPClidarDriver::waitForData(size_t data_count, uint32_t timeout,
...
result_t CSPClidarDriver::checkAutoConnecting() {
...
int CSPClidarDriver::cacheScanData() {
...
result_t CSPClidarDriver::checkDeviceInfo(uint8_t *recvBuffer, uint8_t byte,
...
result_t CSPClidarDriver::waitDevicePackage(uint32_t timeout) {
  int recvPos = 0;
  asyncRecvPos = 0;
  uint32_t startTs = getms();
  uint32_t waitTime = 0;
...
result_t CSPClidarDriver::waitPackage(node_info *node, uint32_t timeout) {
  int recvPos         = 0;
  uint32_t startTs    = getms();
  uint32_t waitTime   = 0;
  ...
  if (CheckSumResult) {
    if (m_intensities) {
      ...
      */
      (*node).sync_quality = ((uint16_t)((packages.packageSampleDistance[package_Sample_Index])&0x03));
      (*node).distance_q2 =
...
result_t CSPClidarDriver::waitScanData(node_info *nodebuffer, size_t &count,
                                     uint32_t timeout) {
  if (!isConnected) {
    count = 0;
    return RESULT_FAIL;
...
result_t CSPClidarDriver::grabScanData(node_info *nodebuffer, size_t &count,
                                     uint32_t timeout) {
...
result_t CSPClidarDriver::ascendScanData(node_info *nodebuffer, size_t count) {
...
/************************************************************************/
result_t CSPClidarDriver::getHealth(device_health &health, uint32_t timeout) {
  result_t ans;
...
/************************************************************************/
result_t CSPClidarDriver::getDeviceInfo(device_info &info, uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
void CSPClidarDriver::setIntensities(const bool &isintensities) {
...
/*  start to scan                                                       */
/************************************************************************/
result_t CSPClidarDriver::startScan(bool force, uint32_t timeout) {
  result_t ans;
...
result_t CSPClidarDriver::stopScan(uint32_t timeout) {
  UNUSED(timeout);
...
result_t CSPClidarDriver::createThread() {
  _thread = CLASS_THREAD(CSPClidarDriver, cacheScanData);
...
result_t CSPClidarDriver::startAutoScan(bool force, uint32_t timeout) {
  result_t ans;
...
/*  reset device                                                        */
/************************************************************************/
result_t CSPClidarDriver::reset(uint32_t timeout) {
  UNUSED(timeout);
  result_t ans;
...
/*  startMotor                                                          */
/************************************************************************/
result_t CSPClidarDriver::startMotor() {
  ScopedLocker l(_lock);
...
/*  stopMotor                                                           */
/************************************************************************/
result_t CSPClidarDriver::stopMotor() {
  ScopedLocker l(_lock);
...
/************************************************************************/
result_t CSPClidarDriver::getScanFrequency(scan_frequency &frequency,
    uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::setScanFrequencyAdd(scan_frequency &frequency,
    uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::setScanFrequencyDis(scan_frequency &frequency,
    uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::setScanFrequencyAddMic(scan_frequency &frequency,
    uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::setScanFrequencyDisMic(scan_frequency &frequency,
    uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::getSamplingRate(sampling_rate &rate, uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::setSamplingRate(sampling_rate &rate, uint32_t timeout) {
  result_t  ans;
...
/************************************************************************/
result_t CSPClidarDriver::getZeroOffsetAngle(offset_angle &angle,
    uint32_t timeout) {
  result_t  ans;
...
std::string CSPClidarDriver::getSDKVersion() {
  return SDKVerision;
}
std::map<std::string, std::string>  CSPClidarDriver::lidarPortList() {
  std::vector<PortInfo> lst = list_ports();
  std::map<std::string, std::string> ports;

  for (std::vector<PortInfo>::iterator it = lst.begin(); it != lst.end(); it++) {
    std::string port = "CSPClidar" + (*it).device_id;
    ports[port] = (*it).port;
  }
